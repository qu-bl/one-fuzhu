...
| `row` | 子组件从左到右排列；空间不足时由宿主自适应，全部可见子组件必须保持可达 | 无 |
| `stack` | 子组件叠加 | `align`、`valign` |

- `align`：`start`、`center`、`end`。
- `valign`：`top`、`center`、`bottom`。
- `scroll`：仅 `column` 可启用原生滚动。该分组取得父级纵向布局的剩余高度，并在其内部使用平台原生滚动。
- 动态 UI 页面本身固定在当前窗口或分栏的可用区域内，不创建整页滚动。窗口尺寸变化时，宿主重新测量所有原生组件并把变化后的剩余高度交给 `scroll=true` 分组。
- 普通兄弟组件保持自然高度，因此可以组成“顶部固定内容 + 中部滚动内容 + 底部固定操作区”。在非滚动的有界 `column` 中，`spacer` 和多个滚动分组共同分配剩余高度。
- `row`：可用宽度不足时，宿主使用平台原生的压缩、文字自然换行或容器换行能力保留全部可见子组件，
  不得把超出单行的后续组件静默裁掉；脚本不声明换行点。
...
- 宿主使用 SwiftUI、Material 3、ArkUI 的原生控件及系统样式，不自行补画边框、背景、圆角、颜色或阴影。
- 宿主可以选择由平台完整定义的原生样式变体，例如 SwiftUI 的 `.roundedBorder`；宿主不得自行指定该样式的边框粗细、颜色、圆角半径或动画。
- 输入、开关、滑杆和选择控件的 `label` 必须由宿主以平台标准的可见标签呈现，并同时保留无障碍名称；按钮显示 `text`，`label` 作为动作语义和无障碍名称。
- 页面、表单、列表和弹窗使用平台原生容器负责安全区、内容边距和行对齐。页面不滚动；只有声明 `scroll=true` 的纵向分组负责内容滚动。原子控件不得自行模拟页面边距。
- 平台系统样式允许不同：例如 Apple 输入框可以使用 `.roundedBorder`，Material 3 与 ArkUI 使用各自平台定义的输入样式。
...
原生 QVMI `list` 字段。`optionsArray` 必须是字符串数组，每个字符串同时作为原生选项的显示文案和值；
不要传入 `{ label, value }` 对象数组。`json` 对象仍建立为 `json` 字段。
