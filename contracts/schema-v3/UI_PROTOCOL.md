...
`filled` 选择原生强调按钮，`outlined/tonal` 优先选择平台对应原生样式；平台没有对应样式时使用
原生普通按钮。`danger` 优先使用平台原生破坏性角色，没有时使用原生强调按钮。`selected=true` 时将
`plain/outlined/tonal` 提升为 `filled`，同时保留 `filled/danger`。按钮不提供图标字段。选择器的
